...
public class RentalObjectService {
    ...
    public RentalObject findRentalObject(int objectId) {
        return rentalObjectRepository.findById(objectId).orElse(null);
    }
...
//        rentalObjectRepository.save(rentalObject);
    ...
    public void insertRentalObject(String userEmail, RentalObjectCmd cmd) {
        UserEntity userEntity = userRepository.findByUserEmail(userEmail).orElse(null);
        SubCategory subCategory = subCategoryRepository.findByScId(cmd.getScId()).orElse(null);

        if (userEntity != null && subCategory != null) {
            Address address = new Address(cmd.getZipCode(), cmd.getRoadAddress(), cmd.getJibunAddress(), cmd.getSido(), cmd.getSigungu());
            RentalObject rentalObject = new RentalObject(userEntity, subCategory, cmd.getObjectName(), cmd.getPrice(), cmd.getWebsite(),
                    cmd.getTarget(), cmd.getLocation(), LocalDate.parse(cmd.getStartDuration()), LocalDate.parse(cmd.getEndDuration()),
                    LocalDate.parse(cmd.getReceiptStartDuration()), LocalDate.parse(cmd.getReceiptEndDuration()), cmd.getCapacity(),
                    cmd.getRepresentNum(), cmd.getUseInfo(), cmd.getLocationInfo(), cmd.getObjectImage(), address);
...
//TODO : 기본키 외래키 바꿔야 할 값을 모두 받아온후에 값 검증을 하기
    ...
    public void updateRentalObject(int objectIndex, String scId, String ownerEmail, String objectName, int price,
                                   ...
                                   LocalDateTime receiptDuration, int capacity, String representNum, String useInfo,
...
//            changeRentalObject.setReceiptDuration(receiptDuration);
            ...
            changeRentalObject.setUseInfo(useInfo);
